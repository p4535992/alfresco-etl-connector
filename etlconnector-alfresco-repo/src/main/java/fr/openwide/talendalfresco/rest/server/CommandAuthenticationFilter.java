...
public class CommandAuthenticationFilter implements Filter {
   ...
   public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
      ...
      if (ticket == null || ticket.length() == 0) {
         ...
         if (session != null) {
            //ticketAuth = (session.getAttribute(AuthenticationUtil.AUTHENTICATION_USER) != null);
        	ticketAuth = (session.getAttribute(AuthenticationUtil.getFullyAuthenticatedUser()) != null);
            //ticketAuth = authenticationService.getCurrentTicket(); 
         }
      } else {
         ticketAuth = true;
      }
